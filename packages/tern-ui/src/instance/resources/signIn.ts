...
type AuthMethodFunction = (auth: Auth, provider: GoogleAuthProvider | OAuthProvider) => Promise<FirebaseAuthResult>;
...
export class SignIn extends TernSecureBase implements SignInResource {
  ...
  private constructor(config: TernSecureConfig) {
    ...
    this.firebaseApp = getApps().length === 0 ? initializeApp(config, appName) : getApps()[0];
  ...
  private async executeAuthMethod(authMethod: AuthMethodFunction, providerName: string): Promise<SignInResponseTree> {
